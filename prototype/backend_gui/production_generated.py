...
def _validate_shot_capability(project_dir: Path, shot_id: str, capability: str) -> None:
    ...
    valid_shots={str(x.get("shot_id")) for x in script.get("entries",[]) if isinstance(x,dict) and x.get("shot_id")}
    if shot_id not in valid_shots: raise ValueError(f"unknown script shot: {shot_id}")
...
def request_generation(project_id: str, *, shot_id: str, capability: str, prompt: str, provider_hint: str="", notes: str="") -> dict:
    ...
    data.setdefault("requests",[]).append(rec); _write_json(path,data)
...
def register_generated(project_id: str, *, asset_id: str, shot_id: str, capability: str, request_id: str="", provider: str="", model: str="", prompt: str="", role: str="generated_visual") -> dict:
    ...
    shot_id=str(shot_id or "").strip(); capability=str(capability or "").strip(); _validate_shot_capability(project_dir,shot_id,capability)
    ...
    with base.LOCK:
        ...
        asset.update({"origin":"generated","kind":capability,"role":str(role or "generated_visual"),"generation":generation,"updated_at":base.now()})
        base.save_state(); asset_copy=dict(asset)
    ...
    if request_id:
        for req in reqs.get("requests",[]):
            if isinstance(req,dict) and req.get("id")==request_id:
                req.update({"status":"fulfilled","asset_id":asset_id,"fulfilled_at":base.now()})
...
def reject_generated(project_id: str, *, asset_id: str, reason: str) -> dict:
    ...
    state["shot_proofs_accepted"]=False; state["final_qc_passed"]=False; _write_json(state_path,state)
    commit=production_project._git_commit_paths(engine,[manifest_path,state_path],"Record generated media rejection"); production_project._clear_guard_marker(engine)
    return {"ok":True,"asset_id":asset_id,"creative_status":"rejected","commit":commit}


def status(project_id: str) -> dict:
    current,_engine,project_dir=_project(project_id); reqs=_read_json(_requests_path(project_id),{"requests":[]}); manifest=_read_json(project_dir/"ASSET_MANIFEST.json",{"assets":[]})
