...
Technical/mode-aware proof readiness is separate from artistic acceptance.
Rejected proofs roll dependent production state back fail-closed.
"""
...
def _proof_path(project_dir: Path,shot_id: str):return project_dir/"shot_proofs"/f"{shot_id}.json"

def _effective_mode(project_dir: Path,shot_id: str,requested: str=""):
    requested=str(requested or "").strip()
    script=_read_json(project_dir/"SCRIPT.json",{})
    entry=next((x for x in script.get("entries",[]) if isinstance(x,dict) and str(x.get("shot_id"))==shot_id),{})
    order=_read_json(project_dir/"OPERATING_ORDER.json",{})
    overall=str(order.get("production_mode") or "").strip()
    if overall=="hybrid":
...
def record_proof(project_id: str,*,shot_id: str,proof_asset_id: str,checks: dict,production_mode: str="",notes: str=""):
    ...
    pkg_path=project_dir/"shot_packages"/shot_id/"package.json"
    ...
    mode=_effective_mode(project_dir,shot_id,production_mode)
    required=MODE_CHECKS[mode]
    ...
    rec={"schema":"aivideoedit.shot-proof.v1","shot_id":shot_id,"production_mode":mode,"package_sha256":_sha(pkg_path),"proof_asset":{"asset_id":proof_asset_id,"uri":f"aive://asset/{proof_asset_id}","sha256":proof_asset.get("sha256"),"content_type":proof_asset.get("content_type"),"name":proof_asset.get("filename")},"checks":{k:bool(checks.get(k)) for k in required},"technical_and_mode_checks_passed":True,"creative_status":"needs_review","notes":str(notes or "").strip(),"recorded_at":base.now()}
    proof_path=_proof_path(project_dir,shot_id);_write_json(proof_path,rec)
    state_path=project_dir/"PROJECT_STATE.json";state=_read_json(state_path,{})
    state["shot_proofs_accepted"]=False;state["mode_aware_proofs_accepted"]=False;_write_json(state_path,state)
    commit=production_project._git_commit_paths(engine,[proof_path,state_path],f"Record proof for {shot_id}");production_project._clear_guard_marker(engine)
    return {"ok":True,"proof":rec,"commit":commit}

def accept_proof(project_id: str,*,shot_id: str,instruction: str):
    ...
    path=_proof_path(project_dir,str(shot_id or "").strip());rec=_read_json(path,{})
    if not rec.get("technical_and_mode_checks_passed"):raise RuntimeError("proof has not passed technical/mode checks")
    rec.update({"creative_status":"accepted","user_acceptance_statement":instruction,"accepted_at":base.now()});_write_json(path,rec)
    commit=production_project._git_commit_paths(engine,[path],f"Accept proof for {shot_id}");production_project._clear_guard_marker(engine)
    return {"ok":True,"proof":rec,"commit":commit}

def finalize_acceptance(project_id: str,*,instruction: str):
    current,engine,project_dir=_project(project_id);instruction=str(instruction or "").strip()
    ...
    missing=[];proofs=[]
    for pkg in packages:
        rec=_read_json(_proof_path(project_dir,pkg.name),{})
    ...
    if missing:raise RuntimeError("all shot proofs must be explicitly accepted: "+", ".join(missing))
    state_path=project_dir/"PROJECT_STATE.json";state=_read_json(state_path,{})
    state.update({"shot_proofs_accepted":True,"mode_aware_proofs_accepted":True,"shot_proof_acceptance_instruction":instruction,"shot_proof_count":len(proofs),"shot_proofs_accepted_at":base.now()});_write_json(state_path,state)
    summary_path=project_dir/"shot_proofs"/"ACCEPTANCE.json";_write_json(summary_path,{"schema":"aivideoedit.shot-proof-acceptance.v1","accepted":True,"instruction":instruction,"shot_ids":[p.get("shot_id") for p in proofs],"accepted_at":base.now()})
    commit=production_project._git_commit_paths(engine,[state_path,summary_path],"Accept complete shot proof set");production_project._clear_guard_marker(engine)
    return {**status(project_id),"commit":commit}

def reject_proof(project_id: str,*,shot_id: str,reason: str):
    ...
    path=_proof_path(project_dir,shot_id);rec=_read_json(path,{})
    if not rec:raise ValueError("proof not found")
    rec.update({"creative_status":"rejected","rejection_reason":reason,"rejected_at":base.now()});_write_json(path,rec)
    state_path=project_dir/"PROJECT_STATE.json";state=_read_json(state_path,{})
    ...
    state["last_creative_rejection"]={"shot_id":shot_id,"reason":reason,"at":base.now()};_write_json(state_path,state)
    commit=production_project._git_commit_paths(engine,[path,state_path],f"Reject proof for {shot_id}");production_project._clear_guard_marker(engine)
    return {**status(project_id),"commit":commit}
