...
def call_tool(
    ...
    arguments: dict | None,
    *,
    dispatch_job: Callable[[dict], None],
    prepare_project: Callable[[str], list[dict]],
) -> dict:
    args = arguments or {}

    if name == "core.status":
        return CORE.status()
    if name == "core.bootstrap":
        return CORE.bootstrap(bool(args.get("offline", False)))
    if name == "capabilities.list":
        status = CORE.status()
        return {"core": status, "capabilities": CORE.capabilities() if status.get("bootstrapped") else []}
    if name == "fx.list":
        status = CORE.status()
        return {"core": status, "effects": CORE.effects() if status.get("bootstrapped") else []}
    if name == "project.list":
        with base.LOCK:
            return {"projects": [dict(p) for p in base.STATE["projects"]]}
    if name == "project.create":
        return {"project": _create_project(str(args.get("name") or ""))}
    if name == "project.status":
        return _project_status(str(args.get("project_id") or ""))
    ...
    if name == "media.prepare":
        ...
        if operation not in {"make_proxy", "extract_review_frames", "qc_media"}:
            raise ValueError("unsupported media preparation operation")
        job = base.add_job(operation, asset.get("project") or "prototype", asset_id)
        dispatch_job(job)
        return {"job": job}
    ...
    if name == "storage.sync":
        ...
        if not storage.status().get("configured"):
            raise ValueError("external storage is not configured")
