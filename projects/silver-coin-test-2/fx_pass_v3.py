...
def main() -> int:
    ...
    ap.add_argument("--manifest", type=Path, default=HERE / "FX_PASS_v3.json")
    ...
    total = starts[-1]
    end = min(args.end_frame or total, total)
    ...
    for scene_id, spec in transition_by_scene.items():
        ...
        frame_no = min(total - 1, starts[si + 1] + 3)
    ...
    try:
        for fi in range(args.start_frame, end):
            ...
            mix = .30 if sid in ("SC19", "SC21") else .42
            ...
            if sid in ("SC19", "SC21"):
                angle = (2.0 if sid == "SC19" else 1.5) * math.sin(local * 1.55)
