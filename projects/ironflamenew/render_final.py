...
def interp_driver(kind,q):
    seq=drivers[kind];x=np.clip(q,0,1)*(len(seq)-1);i=int(x);j=min(len(seq)-1,i+1);a=x-i
    small=cv2.addWeighted(seq[i],1-a,seq[j],a,0);return cv2.resize(small,(W,H),interpolation=cv2.INTER_CUBIC)
...
def render_frame(global_idx):
    t=global_idx/FPS;si=len(recipes)-1
    ...
    ctx=FXContext(local,dur,global_idx,FPS,energy,onset,high);fr=a['base'].copy();drv=interp_driver(r['source_dna'],q);fr=overlay_driver(fr,drv,r['source_dna'],energy)
    ...
    fr=rt.practical_light_breath(fr,ctx,mask=orbmask,strength=.050+.025*onset);fr=rt.moving_light_field(fr,ctx,origin=(pos[0]/W,pos[1]/H),strength=.045+.025*high)
    ...
    dx=1.7*math.sin(local*.29+si*.71);dy=1.1*math.sin(local*.23+.8+si*.37);M=np.float32([[1,0,dx],[0,1,dy]]);return cv2.warpAffine(fr,M,(W,H),flags=cv2.INTER_CUBIC,borderMode=cv2.BORDER_REFLECT_101)
def final_render(out):
    total=int(round(recipes[-1]['end']*FPS));audio=os.environ.get('AIVIDEOEDIT_AUDIO',str(ROOT/'source/Ironflame_Redux.wav'));cmd=['ffmpeg','-y','-hide_banner','-loglevel','error','-f','rawvideo','-pix_fmt','bgr24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-i',audio,'-map','0:v:0','-map','1:a:0','-c:v','libx264','-preset','fast','-crf','17','-pix_fmt','yuv420p','-c:a','aac','-b:a','320k','-shortest','-movflags','+faststart',str(out)];proc=subprocess.Popen(cmd,stdin=subprocess.PIPE)
    for i in range(total):proc.stdin.write(render_frame(i).tobytes())
    proc.stdin.close();rc=proc.wait();
    if rc:raise SystemExit(rc)
if __name__=='__main__':
    import argparse;ap=argparse.ArgumentParser();ap.add_argument('--out',default='/mnt/data/IronFlameNew_FINAL_720p24.mp4');a=ap.parse_args();final_render(Path(a.out))
