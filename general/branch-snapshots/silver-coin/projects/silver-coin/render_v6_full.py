...
silent=OUT/'Silver_Coin_V6_Full_silent.mp4'; writer=cv2.VideoWriter(str(silent),cv2.VideoWriter_fourcc(*'mp4v'),FPS,(RW,RH))
N=int(round(DUR*FPS)); started=time.time(); bounds=[x[0] for x in SCENES[1:]]
for n in range(N):
 t=n/FPS
 ...
 for k,bound in enumerate(bounds):
  ...
  if abs(t-bound)<=w:
   ...
   break
 writer.write(fr)
writer.release(); render_sec=time.time()-started
final=OUT/'Silver_Coin_V6_Full_MusicDirected_720p.mp4'
subprocess.run(['ffmpeg','-y','-hide_banner','-loglevel','error','-i',str(silent),'-i',str(AUDIO),'-t',str(DUR),'-vf',f'scale={DW}:{DH}:flags=lanczos,unsharp=5:5:0.18:3:3:0.0','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','-c:a','aac','-b:a','256k','-ar','48000','-movflags','+faststart',str(final)],check=True)
sha=hashlib.sha256(final.read_bytes()).hexdigest()
meta={
 'title':'Silver Coin V6 full music-directed living painting render','duration_seconds':DUR,'render_seconds':round(render_sec,2),
 'render_base':[RW,RH,FPS],'delivery':[DW,DH,FPS],'canonical_audio':AUDIO.name,'sha256':sha,
 'scene_count':len(SCENES),'source_image_count':len(P),'scene_timeline':[{'start':s,'end':e,'image':P[k].name,'section':sec,'camera_preset':kind} for s,e,k,sec,kind in SCENES],
 'music_controls':['smoothed energy -> light/camera intensity','transient peaks -> micro zoom/light accents','brightness proxy -> atmospheric lift','section grammar -> camera amplitude and palette'],
 'loops':['Gaussian atmosphere drift','tavern practical-light breath','wet-road reflection shimmer','slow camera/perspective loop'],
 'transitions':['clean music-bound dissolve','candle flare','pigment soften','shadow wipe','fog bridge','final light lift'],
 'claim_guard':'Gaussian atmosphere is 2D Gaussian light-field compositing, not 3D Gaussian Splatting; detailed subjects remain static paintings.'
}
(OUT/'V6_FULL_RENDER_META.json').write_text(json.dumps(meta,indent=2))
print(json.dumps({'render_seconds':render_sec,'file':str(final),'sha256':sha,'bytes':final.stat().st_size},indent=2))
