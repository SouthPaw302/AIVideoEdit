...
def robust_z(x):
    x=np.asarray(x,float);med=np.median(x);mad=np.median(np.abs(x-med))+1e-8;return (x-med)/(1.4826*mad)
...
def scan(path,sample_fps=3.0,width=320,timeline_path=None,transition_window=.85):
    ...
    cap.release();
    arr={k:np.array([r[k] for r in rows]) for k in ['sharpness','frame_difference','flow']}
    zs={k:robust_z(v) for k,v in arr.items()}
    boundaries=[]
    if timeline_path:
    ...
    for i,r in enumerate(rows):
        if i==0:continue
        score=max(abs(zs['frame_difference'][i]),abs(zs['flow'][i]),max(0,-zs['sharpness'][i]))
        reasons=[]
        # Robust z-score alone over-flags tiny changes when a render is very stable;
    ...
    unexplained=[x for x in risks if not x.get('expected_transition')]
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('video');ap.add_argument('--json-out',required=True);ap.add_argument('--sample-fps',type=float,default=3.0);ap.add_argument('--timeline');ap.add_argument('--transition-window',type=float,default=.85);a=ap.parse_args();Path(a.json_out).write_text(json.dumps(scan(a.video,a.sample_fps,timeline_path=a.timeline,transition_window=a.transition_window),indent=2))
